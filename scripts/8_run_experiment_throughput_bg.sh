...
echo "  Print Latency: $latency" >> log.txt
...
then
    ...
    echo "Latency value $latency is invalid" >&2
    exit 1
fi

echo "Setting BG_TRAFFIC_INPUT=True" >> log.txt
sshpass -p $TOFINO_USER_PASS ssh $TOFINO_USERNAME@$QUEUEMEM_TOFINO_NAME "mv $QUEUEMEM_PATH/setup.py $QUEUEMEM_PATH/setup.py.bak; sed 's/BG_TRAFFIC_INPUT = .*/BG_TRAFFIC_INPUT = True/g' $QUEUEMEM_PATH/setup.py.bak > $QUEUEMEM_PATH/setup.py"
echo "Setting DEFAULT_N_PAYLOADS=40" >> log.txt
...
do
    ...
    echo "$(date +'%m-%d-%y-%T') - BG Traffic ${mcast}x100Gbps ~ End Run ${i}" >> log.txt
