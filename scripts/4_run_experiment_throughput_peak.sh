...
echo "Setting PEAK=True" >> log.txt
sshpass -p $TOFINO_USER_PASS ssh $TOFINO_USERNAME@$MCAST_TOFINO_NAME "sed -i 's/PEAK_RATE = False/PEAK_RATE = True/g' $MULTICAST_PATH/setup_bq_forwarder.py"
...
do
    ...
    echo "$(date +'%m-%d-%y-%T') - Peak Throughput ${mcast}x100Gbps ~ End Run ${i}" >> log.txt
