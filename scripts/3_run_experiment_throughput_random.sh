...
echo "Setting RANDOM_RATE=True" >> log.txt
sshpass -p $TOFINO_USER_PASS ssh $TOFINO_USERNAME@$MCAST_TOFINO_NAME "sed -i 's/RANDOM_RATE = False/RANDOM_RATE = True/g' $MULTICAST_PATH/setup_bq_forwarder.py"
echo "Setting PER_PORT_STATS=True" >> log.txt
sshpass -p $TOFINO_USER_PASS ssh $TOFINO_USERNAME@$MCAST_TOFINO_NAME "sed -i 's/PER_PORT_STATS = False/PER_PORT_STATS = True/g' $MULTICAST_PATH/setup_bq_forwarder.py"
...
for ((i=1; i<=runs; i++))
do
    echo "$(date +'%m-%d-%y-%T') - Random Throughput ${mcast}x100Gbps ~ Start Run ${i}" >> log.txt
    ...
    echo "$(date +'%m-%d-%y-%T') - Random Throughput ${mcast}x100Gbps ~ End Run ${i}" >> log.txt
