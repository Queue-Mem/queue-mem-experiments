...
sshpass -p $TOFINO_USER_PASS ssh $TOFINO_USERNAME@$MCAST_TOFINO_NAME "sed -i 's/DROP_THRESHOLD = .*/DROP_THRESHOLD = $drops/g' $MULTICAST_PATH/setup_bq_mcast.py"
...
do
    ...
    echo "$(date +'%m-%d-%y-%T') - Trace ${TRACE_FILE} ${mcast}x100Gbps ~ End Run ${i}" >> log.txt
